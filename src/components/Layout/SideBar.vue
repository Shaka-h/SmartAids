<template>
  <div
    class="p-3 css-selector flex-col font-bold hidden sm:flex"
    style="width: 280px"
  >
    <div
      class="capitalize name border-b pt-2 pb-3 text-center mb-2 font-bold"
      style="color: #0d1042"
    >
      AlphaConnect
    </div>
    <div
      class="flex space-x-4 mt-4 items-center"
      v-if="profileContract != 0x0000000000000000000000000000000000000000"
    >
      <button
        type="button"
        class="border hover:font-bold btn border-white px-2 mr-3 text-white rounded-md"
      ...
      <div v-if="profile" class="rounded-md shadow intro-x">
        <img
          :src="`http://127.0.0.1:8080/ipfs/${profileImage[0]?.photoCID}`"
    ...
    <div class="mt-8 h-full space-y-4">
      <template v-for="(navigationLink, index) of navigationLinks" :key="index">
        <template v-if="!navigationLink?.children?.length">
          ...
          <div
            class="flex items-center rounded-md cursor-pointer inactive intro-x"
            v-else
          ...
          >
            <span
              class="bg-primary2 h-8 rounded-r"
              style="width: 4px"
              v-if="hoveredLink === index"
            ></span>
        ...
        <template v-else>
          <div
            class="px-3 intro-x mt-3 italic text-sm font-bold"
            style="color: #707070"
          >
            {{ navigationLink?.name }}
          </div>
          ...
          <template
            v-for="(
              childNavigationLink, childIndex
            ) of navigationLink?.children"
            :key="childIndex"
          >
            <div
              class="flex items-center rounded-md"
              v-if="isActive(childNavigationLink?.link)"
              style="background-color: #c6deff"
            >
              <span class="bg-primary2 h-8 rounded-r" style="width: 4px"></span>
              <div class="flex items-center p-2 text-primary2">
                <svg-icon
                  :name="childNavigationLink?.icon"
                  height="h-5"
                  width="w-5"
                  class="icon"
                  color="#257ae4"
            ...
            >
              <span
                class="bg-primary2 h-8 rounded-r"
                style="width: 4px"
                v-if="childHoveredLink === `${index}_${childIndex}`"
              ></span>
              <span class="h-8 rounded-r" style="width: 4px" v-else></span>
              <div class="flex items-center p-2">
                <svg-icon
      ...
      <div
        v-if="profileContract != 0x0000000000000000000000000000000000000000"
        class="p-2 mt-8 mx-8 cursor-pointer"
      >
...
onMounted(async () => {
  const getprofileContract =
    await nftProfileFactory_contract.profileByAddressOwner(
      route?.params?.wallet
    );
